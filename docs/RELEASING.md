...
| 2 | **macOS** (README, "Platform status") says "Covered by CI only" | The `macos-latest` job is green on the commit that is tagged | The CI run of that exact commit | Stays as it is: nobody has used Bifrost on a Mac by hand. If someone does, say so and by whom |
