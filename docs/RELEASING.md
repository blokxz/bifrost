...
|---|---|---|---|---|
| 1 | **Windows** (README, "Platform status") says "Tested by hand on Windows" (done) | You run the Windows part of `docs/manual-tests.md` on the physical Windows machine, with the real `ssh.exe`, and each step passes or its failure is recorded as a known limitation | The results of every step in the "Windows" section, written down. Anything that failed is fixed or listed under "Known limitations" in `docs/DECISIONS.md` and in the README | Run 2026-09-22 from the `v0.1.0-rc.1` archive, with OpenSSH 9.5p2, against the Incus container; the results are in `docs/manual-tests.md`, "Results, 2026-09-22". Every step passed, and the four things found (the "Unknown error" classification, the Include line inside the exported file, which file the export screen names, and an include loop reported as healthy) were fixed in the same block. README changed from "Builds and passes the automated tests in CI; manual testing pending." to "Tested by hand on Windows, with the real OpenSSH client, as well as in CI." |
| 2 | **macOS** (README, "Platform status") says "Covered by CI, including the terminal tests" (done) | The `macos-latest` job is green on the commit that is tagged, **with the pseudo-terminal suites (`pty`, `pty_connect`, `pty_keys`, `pty_sshconfig`) run and passing, not skipped**. They are the only test of Bifrost's terminal handling on macOS: raw mode, the alternate screen, resizes, what happens when ssh takes the terminal | The CI run of that exact commit: open the macOS job's log and read the result line of each of those four suites | Checked on commit `8bc8dcd` (2026-09-22, CI run 35737879539): `pty` 14/14, `pty_connect` 38/38, `pty_keys` 44/44 (1 ignored, the real-tool test), `pty_sshconfig` 13/13, none skipped. README changed from "Covered by CI only" to "Covered by CI, including the terminal tests" in the same commit as this check. Re-check if the tagged commit ever differs from `8bc8dcd`, or if the pty suites are ever limited to Linux |
